...
public class PowerapiMojo extends AbstractMojo {
    ...
    private void fillTestAndPowerapiData() {
        String[] cmd = {"sh", "-c", "(mvn test -DforkCount=0 | grep timestamp= | cut -d '-' -f 2 | tr -d ' ') > test1.csv & echo pid: $!; (powerapi duration 30 modules procfs-cpu-simple monitor --frequency 50 --console --all | grep muid) > data1.csv;"};
